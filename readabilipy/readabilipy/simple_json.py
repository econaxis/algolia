...
def simple_json_from_html_string(html, content_digests=False, node_indexes=False, use_readability=False):
    ...
    if use_readability:
        ...
        html_path = os.path.join(temp_dir, "full{}.html".format(str(random.randint(0, 0xFFFFFFF))))
        with open(html_path, 'w') as f:
            f.write(html)

        # Call Mozilla's Readability.js Readability.parse() function via node, writing output to a temporary file
        article_json_path = os.path.join(temp_dir, "article{}.json".format(str(random.randint(0, 0xFFFFFFF))))
